        ...
        if #available(iOS 15.0, *) {
            ...
            { image in
                image
                    ...
                    .padding(.bottom, -15).frame(width: 100, height: 150, alignment: .center)
            } placeholder: {
                placeholderImage()
            }
        } else {
            KFImage(URL(string: urlToLoadFix)!).resizable().aspectRatio(contentMode: .fit).clipShape(RoundedRectangle.init(cornerRadius: 4)).padding(.bottom, -15).frame(width: 100, height: 150, alignment: .center)
        }
    }
}
...
struct ImageByUrl_Previews: PreviewProvider {
    static var previews: some View {
        ImageByUrl(urlToLoad: "http://http2.mlstatic.com/D_943484-MLA46651720937_072021-I.jpg")
    }
}
